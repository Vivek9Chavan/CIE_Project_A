# -*- coding: utf-8 -*-
"""
Code for Presentation 20.11.2019

Project A1/Team 28

@authors: Ahmet Küpeli, Vivek Chavan, Chi Shing Li
...
Smartphones were placed in the rear pockets of students who did two sets of walks:
1. Normal walking
2. Impaired walking (due to a block placed in one shoe)

The aim the project was to develop a machine learning model that discerns between the two categories effectively.
...
Total =glob.glob('C:/Users/Vivek/sciebo/CIE/Smartphone1/subject*')

Subject=[]
counter=-1
for i in range(1,22):
    i_str=str(i).zfill(2)# Because otherwise it doesnt work when it reaches 10, turns single digits to 2 digits: 1-> 01
    #Subject.append(glob.glob('/home/charles/ownCloud/cie/Smartphone1s/subject*' + str(i_str) +'*'))
    Subject.append(glob.glob('C:/Users/Vivek/sciebo/CIE/Smartphone1/subject*' + str(i_str) + '_' + '*'))
...
for i in range(0,len(Total)):
    ...
    #This is commented when cut data is used, uncomment this and commend line 275-362
...
#    for x in range(0,len(seq_sam_x)):
#        def rmse(predictions, targets):
#            return np.sqrt(((predictions - targets) ** 2).mean())
#        # Calculate a mean sequence out of all sequences in totat step lengths
#        mean_x = [sum(x) for x in zip(*seq_sam_x)]
#        mean_x = [x /len(seq_sam_x) for x in mean_x]
##        t = range(0,101,1)
##        plt.plot(t, mean_x)
#        # Calculate rmse which show how differences between mean sequence and all sequences
#        rmse_x.append(rmse(mean_x, seq_sam_x[x]))
#       
#        # Remove the whole sequence of step length if rmse> or < than average of rmse
#        # Problem here: we only know which rmse belong to the step is removed. So, need to find the index.
#        rmse_x_new = ([x for x in rmse_x if (x > 1*np.mean(rmse_x))])     
#        rmse_x_new = ([x for x in rmse_x if (x < 1*np.mean(rmse_x))])
#        
#        if rmse_x[x] in rmse_x_new:
#            continue
#        else:
#            cut_x.append(x)
#    
...
#                 continue
#        else:
#           sam_x_new.append(seq_sam_x[x])
#           
#    
#    for x in range(0,len(seq_sam_y)):           
#        if x in cut_x:
#                 continue
#        else:
#           sam_y_new.append(seq_sam_y[x])
#           
#
#    
#    for x in range(0,len(seq_sam_z)):
#        if x in cut_x:
#                 continue
#        else:
#           sam_z_new.append(seq_sam_z[x])
#           
#    
#        
#### Saving all sequented acceleration into one list steps
#    for x in range(0,len(sam_x_new)):
#       steps_loop.append(np.vstack((sam_x_new[x],sam_y_new[x],sam_z_new[x])).T)
#
#    
#    steps.append(steps_loop)   
...
for train_index, test_index in kf.split(Subject):
    ...
    model = keras.Sequential([keras.layers.Flatten(input_shape=(101, 3)), keras.layers.Dense(35, activation='sigmoid'), keras.layers.Dense(2, activation='softmax')])
    ...
    model.fit(train_step, impaired_steps_train, epochs=800 ,shuffle=True);
    #model.fit(train_step, impaired_steps_train, epochs=2000);
...
#print('average accuracy for  Fold CV : ' + str(np.mean(accuracies)))
    ...
    #results_acc.append(accuracies)
    #results_loss.append(losses)
    #

    
    #And then play around with the data
